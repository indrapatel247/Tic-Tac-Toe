...
public class BoardView extends View {
    ...
    private static final int ELT_STROKE_WIDTH = 16;
    ...
    public void setboardEngine(boardEngine g) {
        boardEngine = g;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        eltW = (width - LINE_THICK) / 4;
        eltH = (height - LINE_THICK) / 4;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!boardEngine.isEnded()  &&  event.getAction() == MotionEvent.ACTION_DOWN) {
            int x = (int) (event.getX() / eltW);
            int y = (int) (event.getY() / eltH);
            char win = boardEngine.play(x, y);
            invalidate();
    ...
    private void drawBoard(Canvas canvas) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                drawElt(canvas, boardEngine.elt(i, j), i, j);
            }
        }
    }
